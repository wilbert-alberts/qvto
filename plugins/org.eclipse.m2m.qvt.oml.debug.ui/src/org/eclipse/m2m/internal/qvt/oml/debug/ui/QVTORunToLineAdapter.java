...
public class QVTORunToLineAdapter implements IRunToLineTarget {
	...
	public void runToLine(IWorkbenchPart part, ISelection selection, ISuspendResume target) throws CoreException {
		...
		if(input == null) {
			throw new CoreException(QVTODebugUIPlugin.createErrorStatus(DebugUIMessages.QvtRunToLineAdapter_NoInput));
		}
		...
		if(document == null) {
			throw new CoreException(QVTODebugUIPlugin.createErrorStatus(DebugUIMessages.QvtRunToLineAdapter_NoDocument));
		}
		
		IFile file = (IFile)input.getAdapter(IFile.class);
		if (file == null) {
			throw new CoreException(QVTODebugUIPlugin.createErrorStatus(DebugUIMessages.QvtRunToLineAdapter_NoFile)); 
		}
		
		ITextSelection textSelection = (ITextSelection) selection;
		int lineNumber = textSelection.getStartLine() + 1;
		
		// FIXME use breakpoint location verifier class
		URI resourceURI = URIUtils.getResourceURI(file);
