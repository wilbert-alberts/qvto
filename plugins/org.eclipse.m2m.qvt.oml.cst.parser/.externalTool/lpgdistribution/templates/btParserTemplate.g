...
$Headers
    ...
    public class $action_class implements RuleAction$additional_interfaces
    {
        ...
        {
            int firsttok = prsStream.getFirstErrorToken(error_token),
                lasttok = prsStream.getLastErrorToken(error_token);
            String location = prsStream.getFileName() + ':' +
                              (firsttok > lasttok
                                        ? (prsStream.getEndLine(lasttok) + ":" + prsStream.getEndColumn(lasttok))
                                        : (prsStream.getLine(error_token) + ":" +
                                           prsStream.getColumn(error_token) + ":" +
                                           prsStream.getEndLine(error_token) + ":" +
                                           prsStream.getEndColumn(error_token)))
                              + ": ";
            prsStream.reportError((firsttok > lasttok ? ParseErrorCodes.INSERTION_CODE : ParseErrorCodes.SUBSTITUTION_CODE), location, msg);
        }
